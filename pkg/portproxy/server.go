...
func (p *PortProxy) handleEvent(conn net.Conn) {
	...
	if err = p.execListener(pm); err != nil {
		logrus.Error(err)
	}
}

func (p *PortProxy) execListener(pm types.PortMapping) error {
	for portMapping := range pm.Ports {
		port := portMapping.Int()
		p.mutex.Lock()
		defer p.mutex.Unlock()
		if pm.Remove {
			if listener, exist := p.activeListeners[port]; exist {
				if err := listener.Close(); err != nil {
					logrus.Errorf("error closing listener for port [%d]: %s", port, err)
				}
			}
			continue
		}
		addr := net.JoinHostPort("localhost", portMapping.Port())
		l, err := net.Listen("tcp", addr)
		if err != nil {
			logrus.Errorf("failed creating listener for published port [%d]: %s", port, err)
			continue
		}
		p.activeListeners[port] = l
		go p.acceptTraffic(l, portMapping.Port())
